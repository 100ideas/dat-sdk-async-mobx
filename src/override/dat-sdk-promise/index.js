// dat-sdk/promise
//   modified to avoid non-browser-friendly deps
//
// src: https://github.com/datproject/sdk/blob/master/promise.js
  ...
  class DatArchive extends EventTarget {
    ...
    async download(filepath, opts = {}) {
      ...
      return timer(to(opts), async checkin => {
        ...
        if (this._version)
          throw new Error(
            "Not yet supported: can't download() old versions yet. Sorry!"
          ); // TODO
    ...
    static async selectArchive(options) {
      ...
      const info = await Promise.all(
        archives.map(archive => archive.getInfo())
      );

      const message = `
   Please choose a Dat Archive:
   ${info
     .map(({ url, title }, index) => `${index}. ${title || "Untitled"}: ${url}`)
     .join("\n")}
   `;
    ...
    static async create(options = {}) {
      ...
      await pda.writeManifest(archive._archive, {
        url: archive.url,
        title,
        description,
        type,
...
const to = opts =>
  opts && typeof opts.timeout !== "undefined" ? opts.timeout : API_TIMEOUT;
